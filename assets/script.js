...
this.x = clamp(this.x, -overflow, BASE_W - this.w + overflow);
    ...
    draw(g){
  const h = Math.min(this.h, this.spriteHCapPx);
  const w = this.w; 
  ...
  constructor(game){
    ...
    this.frameDuration = 0.17; // durée d'une frame d'animation
  ...
  applyCaps(){
    ...
    this.h = Math.min(Math.floor(BASE_H * 0.18), maxH); // ≤20%
  }
...
spawnY(){ 
  // juste sous la main
  return (this._y||0) + (this._drawH||48) - 96;
}
